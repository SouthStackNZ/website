exports.handler = async function(event) {
  ...
  // Basic rate limit check — Netlify doesn't do this natively,
  // but you can add more logic here if needed
  ...
  // Only pass through what we need — never let the client override the model or system prompt
  var messages = body.messages;
  if (!messages || !Array.isArray(messages)) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Missing messages' }) };
  }

  // Cap conversation history to last 10 messages so nobody pumps tokens
